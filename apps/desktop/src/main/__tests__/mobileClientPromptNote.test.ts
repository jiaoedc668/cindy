...
describe('stripMainOnlySendOpts(直连路径消毒)', () => {
  ...
  it('strips nested meeting authors without mutating the input', () => {
    const opts = { persistUserMessage: { clientId: 'message', content: 'text', meetingAuthor: { accountId: 'forged' } } };
    expect(stripMainOnlySendOpts(opts)).toEqual({ persistUserMessage: { clientId: 'message', content: 'text' } });
    expect(opts.persistUserMessage.meetingAuthor).toEqual({ accountId: 'forged' });
  });
