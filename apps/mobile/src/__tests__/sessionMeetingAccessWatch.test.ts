...
import { watchSessionMeetingAccess } from '@/device-link/sessionMeetingAccessWatch';

const active = { meetingId: 'meeting', sessionId: 'session', status: 'active' };
...
function start(read: () => Promise<typeof active>, isCurrent = () => true) {
  ...
  const stop = watchSessionMeetingAccess({ ...active, read, isCurrent, onRevoked });
  disposers.push(stop);
